...
module.exports = class CodotypeGenerator {
  ...
  writeEach ({ schema, blueprint }) {
    // console.log('WRITE EACH');
    // console.log(schema.label);
    // console.log(blueprint.identifier);
  }
  ...
  async composeWith (modulePath, options={}) {
    ...
    try {
      ...
      await Promise.all(this.options.blueprint.schemas.map((schema) => generator.writeEach({ schema: schema, ...this.options })))
