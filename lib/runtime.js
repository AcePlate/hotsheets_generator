...
module.exports = class CodotypeRuntime {
  ...
  async execute ({ app, build }) {

    // Inflates application metadata
    app = inflate({ app })

    // Runs stage of the build array
    build.stages.forEach(async ({ generator_path, project_path, configuration }) => {

      // Assigns `dest` option for generator
      const dest = path.join(this.options.cwd, OUTPUT_DIRECTORY, app.identifier, project_path)
